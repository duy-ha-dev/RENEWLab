...
# =============================================================================
    ...
    print(csi.shape)
    print('Printing CSI Data ################!!!!!!!!!!!!!!!!!')
    print(csi[1, 0, user_plt, 0, :, 1])
    ...
    for i in range(csi.shape[4]):
            ...
            amplitude_value_csi.append(np.mean(np.abs(csi[:, 0, user_plt, 0, i, :]), axis =1))
            
            # Phase
            phase_value.append(np.mean(np.angle(samps[:, 0, user_plt, 0, i, :], deg=True), axis = 1))
            phase_value_csi.append(np.mean(np.angle(csi[:, 0, user_plt, 0, i, :], deg=True), axis = 1))
            
            # Raw
            #raw_samp.append(np.mean(samps[:, 0, user_plt, 0, i, :], axis = 1))
            raw_samp.append(samps[:, 0, user_plt, 0, i, :1].flatten())
            raw_samp_csi.append(csi[:, 0, user_plt, 0, i, 30:31].flatten())
    ...
    # amp_values_csi.to_csv('data_ml/amplitude/amp_first_d.csv')
    # print("^^^^^^^ amplitude shaking data saved")
        
    # phase_values_csi = pd.DataFrame(phase_value_csi, columns = [k for k in range(1,2001)])
    # phase_values_csi.to_csv('data_ml/phase/phase_first_d.csv')
    # print("###    Save Phase values shake")
    ...
    # raw_csi_1 = pd.DataFrame(raw_csi, columns = [k for k in range(1,2001)])
    # print('raw_csi_1 &&&&&&&&&&&&&&&&&&&###################')
    # print(raw_csi_1)
    # #raw_samps_csi.to_csv('data_ml/ph/phase_first_d.csv')
    # #print("###    Save Phase values shake")
    ...
    # print('\n')
    # print("Length ##############")
    # print(raw_samps_csi.shape)
        
    # print("Phase ##############")
    # print(phase_values_csi.shape)
...
# =============================================================================
    ...
    #parser.add_option("--analyze-trace", action="store_true", dest="analyze", help="Run script without analysis", default=True)
